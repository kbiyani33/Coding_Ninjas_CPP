...
void DFS(vector< vector<char> > &board, vector< vector<bool> > &visited, int x, int y, int fromx, int fromy, char currentChar, bool &cyclic, int n, int m){
    if(x < 0 || x >= n || y < 0 || y >= m)
        return;
    ...
    for(int i=0; i<4; i++){
        ...
        DFS(board, visited, newX, newY, x, y, currentChar, cyclic, n, m);
    }
}

bool hasCycle(vector< vector<char> > &board, int n, int m) {
    // Write your code here.
    vector< vector<bool> > visited(n, vector<bool>(m, false)); // creating the visited array and marking everything as false
    bool cyclic = false;
    for(int i=0; i<n; i++){
        for(int j=0; j<m; j++){
            // visited[i][j] = true;
            if(!visited[i][j]){
                DFS(board, visited, i, j, -1, -1,  board[i][j], cyclic, n, m);
            }
        }
    }
    return cyclic;
